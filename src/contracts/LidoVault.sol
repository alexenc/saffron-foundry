...
contract LidoVault is ILidoVaultInitializer, ILidoVault {
    ...
    mapping(address => uint256[])
        public fixedToVaultNotStartedWithdrawalRequestIds;
    ...
    mapping(address => WithdrawalRequest)
        public fixedToVaultOngoingWithdrawalRequestIds;
    ...
    function getFixedOngoingWithdrawalRequestTimestamp(
        address user
    ) public view returns (uint256) {
        return fixedToVaultOngoingWithdrawalRequestIds[user].timestamp;
    }

    function getFixedOngoingWithdrawalRequestIds(
        address user
    ) public view returns (uint256[] memory) {
        return fixedToVaultOngoingWithdrawalRequestIds[user].requestIds;
    }

    function getFixedToVaultNotStartedWithdrawalRequestIds(
        address user
    ) public view returns (uint256[] memory) {
        return fixedToVaultNotStartedWithdrawalRequestIds[user];
    }

    function getVariableToVaultOngoingWithdrawalRequestIds(
        address user
    ) public view returns (uint256[] memory) {
        return variableToVaultOngoingWithdrawalRequestIds[user];
    }
    // @audit - crit? possible lock of user funds if gas bomb attack due to array iteration
    ...
    mapping(address => uint256[])
        public variableToVaultOngoingWithdrawalRequestIds;
    ...
    mapping(address => uint256)
        public variableToWithdrawnStakingEarningsInShares;
    ...
    // how that  works in clone contracts? can that lead to initialization errs?
    ...
    function initialize(InitializationParams memory params) external {
        ...
        require(
            params.fixedSideCapacity.mulDiv(minimumFixedDepositBps, 10_000) >=
    ...
    /// @notice Function to receive Ether. msg.data must be empty // @audit - q no check for msg.data emptyness
    ...
    function deposit(uint256 side) external payable {
        ...
        require(!isStarted(), "DAS"); // only callable if not started
        require(side == FIXED || side == VARIABLE, "IS");
        require(msg.value >= minimumDepositAmount, "MDA"); // @audit q we require min deposit and ther another min deposit?
        // @audit - gas cache amount before require
        uint256 amount = msg.value;
        if (side == FIXED) {
            // Fixed side deposits
            // @audit gas vaue can be cached on initialiation
            uint256 minimumFixedDeposit = fixedSideCapacity.mulDiv(
            ...
            // no refunds allowed
            require(
                amount <= fixedSideCapacity - fixedETHDepositTokenTotalSupply,
                "OED"
            );
            // do not allow remaining capacity to be less than minimum fixed deposit bps
            uint256 remainingCapacity = fixedSideCapacity -
                fixedETHDepositTokenTotalSupply -
                amount;
            require(
                remainingCapacity == 0 ||
                    remainingCapacity >= minimumFixedDeposit,
                "RC"
            );

            // Stake on Lido
            /// returns stETH, and returns amount of Lido shares issued for the staked ETH
            uint256 stETHBalanceBefore = stakingBalance(); // balance of stEht of vault
            ...
            // stEthReceived has to be == msg.value +- 10wei
            require(
                (stETHReceived >= amount) ||
                    (amount - stETHReceived <= LIDO_ERROR_TOLERANCE_ETH),
                "ULD"
            );
            // some stEth can be locked due to tolerance
            ...
            uint256 remainingCapacity = variableSideCapacity -
                variableBearerTokenTotalSupply -
                amount;
            // can I make that require to always trigger?
            require(
                remainingCapacity == 0 ||
                    remainingCapacity >= minimumDepositAmount,
                "RC"
            );
        ...
        if (
            // audit q cain i force this conditions to hold true and cause DOS attack??
    ...
    function claimFixedPremium() external {
        ...
        uint256 sendAmount = claimBal.mulDiv(
            variableSideCapacity,
        ...
        (bool sent, ) = msg.sender.call{value: sendAmount}(""); // follows CEI pattern
        require(sent, "ETF");
    ...
    function withdraw(uint256 side) external {
        // @audit gas - move side check to an internal func of modifier
        require(side == FIXED || side == VARIABLE, "IS");
        ...
        if (!isStarted()) {
            if (side == FIXED) {
                ...
                fixedETHDepositTokenTotalSupply -= fixedETHDepositToken[
                    msg.sender
                ];
                ...
                fixedToVaultNotStartedWithdrawalRequestIds[
                    msg.sender
                ] = requestWithdrawViaShares(msg.sender, claimBalance);
                ...
                require(sent, "ETF");
                // @audit - events inconsistency if on call user completes remaining variable amount
                emit VariableFundsWithdrawn(
                    sendAmount,
                    msg.sender,
                    false,
                    false
            ...
            if (side == FIXED) {
                ...
                uint256 fixedETHDeposits = fixedSidestETHOnStartCapacity; // variable initialized on deposit:vaultstarted
                uint256 withdrawAmount = fixedETHDeposits.mulDiv(
                    fixedBearerToken[msg.sender],
                    fixedLidoSharesTotalSupply()
                ); // @audit - gas use bearerBalance to save sload q can this be modified?
                uint256 lidoStETHBalance = stakingBalance();
                // with draw amount double calc can be saved by doing if statement before
                if (fixedETHDeposits > lidoStETHBalance) {
                    ...
                    withdrawAmount = lidoStETHBalance.mulDiv(
                        fixedBearerToken[msg.sender],
                        fixedLidoSharesTotalSupply()
                    );
                }
                ...
                fixedToVaultOngoingWithdrawalRequestIds[
                    ...
                    requestIds: requestWithdrawViaETH(
                        msg.sender,
                        withdrawAmount
                    ), //wamount = stEthamount
                ...
                if (
                    msg.sender == protocolFeeReceiver && appliedProtocolFee > 0
                ) {
                    require(
                ...
                if (
                    ...
                    if (
                        lidoStETHBalance >
                        fixedETHDeposits + minStETHWithdrawalAmount()
                    ) {
                        ...
                        (
                            ...
                            uint256 ethAmountOwed
                                ...
                                variableToWithdrawnStakingEarningsInShares[
                                    msg.sender
                                ].mulDiv(lidoStETHBalance, currentStakes)
                            );
                        if (ethAmountOwed >= minStETHWithdrawalAmount()) {
                            ...
                            uint256 protocolFee = ethAmountOwed.mulDiv(
                                protocolFeeBps,
                                10000
                            );
                            totalProtocolFee += protocolFee;
                            uint256 stakesAmountOwed = lido
                                .getSharesByPooledEth(ethAmountOwed);
    ...
    // @audit - medium not check for vault started stae - ok that is kind of implicit to request type
    function finalizeVaultNotStartedFixedWithdrawals() external {
        uint256[]
            memory requestIds = fixedToVaultNotStartedWithdrawalRequestIds[
                msg.sender
            ];
        ...
        // @audit info gas saving by only calling isStarted() once and the using its inverse
        bool _isStarted = isStarted();
        bool _isEnded = isEnded();
        transferWithdrawnFunds(msg.sender, sendAmount);

        emit LidoWithdrawalFinalized(
            msg.sender,
            requestIds,
            FIXED,
            _isStarted,
    ...
    function finalizeVaultOngoingFixedWithdrawals() external {
        ...
        bool _isEnded = isEnded();
        uint256 arrayLength = fixedOngoingWithdrawalUsers.length; // @audit - high if we iterate this array trhoug all user that means a user can DOS ther users
    ...
    // @audit - med no check for vault current state, q is that derived to initialization of variable...[msgsender] mapping??
    function finalizeVaultOngoingVariableWithdrawals() external {
        uint256[]
            memory requestIds = variableToVaultOngoingWithdrawalRequestIds[
                msg.sender
            ]; // here we get a array
        ...
        uint256 sendAmount = amountWithdrawn +
    ...
    // @audit no check for vault started state
    function feeReceiverFinalizeVaultOngoingVariableWithdrawals(
        address user
    ) external {
        ...
        uint256 sendAmount = amountWithdrawn +
            calculateVariableFeeEarningsShareWithUser(user) -
            protocolFee;
    ...
    // Audit sus a lot of arr iterations
    function finalizeVaultEndedWithdrawals(uint256 side) external {
        ...
        require(
            vaultEndedWithdrawalRequestIds.length != 0 &&
        ...
        uint256 amountWithdrawn = claimWithdrawals(
            msg.sender,
            vaultEndedWithdrawalRequestIds
        ); // q can i make that arr very big?
    ...
    // @audit - high definetly gas bomb as an external call is being performed
    // check length of fixedOngoinWithdrawalUsers
    // if one of the user requestIds length is 0 function will revert
    ...
    function claimOngoingFixedWithdrawals() internal {
        ...
        for (uint i = 0; i < arrayLength; i++) {
            ...
            // give user rights to wihdraw its money (lido eht - protocol fees )
            // this rights are then claimed by vaultEndedWithdraw() func when fixed side
    ...
    function vaultEndedWithdraw(uint256 side) internal {
        if (
            ...
            if (stakingBalance() < minStETHWithdrawalAmount()) {
                // function with nested for loops called
                claimOngoingFixedWithdrawals();
                ...
                vaultEndingStakesAmount = stakingShares();
                vaultEndingETHBalance = stakingBalance();
                vaultEndedWithdrawalRequestIds = requestEntireBalanceWithdraw(
                    msg.sender
                );
        ...
        if (side == FIXED) {
            ...
            if (sendAmount > 0) {
                ...
                require(bearerBalance > 0, "NBT");
                // @audit gas use bearerBalance variable cached before
            ...
            require(
                variableToVaultOngoingWithdrawalRequestIds[msg.sender].length ==
                    0,
                "WAR"
            );
    ...
    function claimFixedVaultOngoingWithdrawal(
        ...
        WithdrawalRequest
            memory request = fixedToVaultOngoingWithdrawalRequestIds[user];
        ...
        // @audit wtf is doing that comment here?
        // uint256 arrayLength = fixedOngoingWithdrawalUsers.length;
        // for (uint i = 0; i < arrayLength; i++) {
        ...
        uint256 earlyExitFees = calculateFixedEarlyExitFees(
            upfrontPremium,
            request.timestamp
        );
    ...
    function calculateVariableWithdrawState(
        ...
        uint256 totalOwed = bearerBalance.mulDiv(
            totalEarnings,
            variableSideCapacity
        );
    ...
    function getCalculateVariableWithdrawStateWithStakingBalance(
        address user
    ) public view returns (uint256) {
        uint256 lidoStETHBalance = stakingBalance();
        uint256 fixedETHDeposits = fixedETHDepositTokenTotalSupply;
        require(lidoStETHBalance > fixedETHDeposits, "LBL");
        uint256 totalEarnings = (lidoStETHBalance - fixedETHDeposits) +
            withdrawnStakingEarnings +
            totalProtocolFee;
        uint256 previousWithdrawnAmount = variableToWithdrawnStakingEarnings[
            user
        ].mulDiv(10000, 10000 - protocolFeeBps);
    ...
    function getCalculateVariableFeeEarningsShareWithUser(
        address user
    ...
    /// @return amount The amount to give the user
    function calculateVariableFeeEarningsShareWithUser(
        address user
    ) internal returns (uint256) {
        (
            uint256 currentState,
    ...
    function calculateFixedEarlyExitFees(
        ...
        uint256 remainingProportion = (
            endTime > timestampRequested ? endTime - timestampRequested : 0
        ).mulDiv(1e18, duration);
        ...
        earlyExitFees +=
            upfrontPremium -
            upfrontPremium.mulDiv(timestampRequested - startTime, duration);
    ...
    function requestWithdrawViaShares(
        address user,
        uint256 shares
    ...
    /// @param user Address of the user that is requesting the withdraw
    /// @return requestIds Ids of the withdrawal requests
    function requestEntireBalanceWithdraw(
        address user
    ) internal returns (uint256[] memory) {
    ...
    /// @return requestIds Ids of the withdrawal requests
    function requestWithdrawViaETH(
        address user,
        uint256 stETHAmount
    ) internal returns (uint256[] memory) {
    ...
    // requestId of lido and gets back eth this is an async process handled by lido
    ...
    function _claimWithdrawals(
        ...
        // this will fail if the request is not finalize
        // @audit critical - if gas bomb this user loss funds!!! do a poc eulation a lot of user 1 wei deposits to cause that
    ...
    function calculateWithdrawals(
        ...
        withdrawalAmounts = new uint256[](
            numberOfWithdrawals + (remainingAmount > 0 ? 1 : 0)
        );
